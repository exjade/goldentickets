...
exports.WeeklySorteoAutomaticoDolarHoraV2 = functions.pubsub.schedule('0 20 * * FRI').timeZone('America/Mexico_City').onRun(async (context) => {
    try {
        ...
        // 3. Obtiene el premio acumulado sumando el costo de los tickets
        const premioRef = db.collection('loteria').doc('weekly-prize');
        const premioData = await premioRef.get();
        ...
        if (!comprasSnapshot.empty) {
            ...
            // 9-10. Almacenar un registro de ganadores y úlimas loterias
            const winnersLogs = db.collection('weekly-lottery-winners').add(informaciónGanador);
            const lotteryLogs = db.collection('weekly-last-lotteries').add(informaciónSorteo);


            // 12. Después del sorteo, elimina la colección "compras"
            await db.collection('loteria').doc('weekly-prize').collection('compras').get().then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
                    doc.ref.delete();
                });
            });
            ...
            const cumulativePrize = db.collection('weekly-accumulated-prize').add(informaciónSorteo);
